 ...
 * @param {number} positiveSentimentThreshold - The threshold above which a positive sentiment score triggers a rank boost (default is 1).
 * @param {number} negativeSentimentThreshold - The threshold below which a negative sentiment score triggers a rank boost (default is -1).
 * @param {number} positiveRankBoost - The multiplier applied to the base TextRank score for sentences with positive sentiments above the threshold (default is 2).
 * @param {number} negativeRankBoost - The multiplier applied to the base TextRank score for sentences with negative sentiments below the threshold (default is 1).
 *
 * @returns {string} A string that concatenates the top-ranked sentences to form the summary.
 *
 * The function works by initializing TextRank and sentiment analysis using the 'natural' library. It first tokenizes the text into sentences,
...
function sentimentSummary(
  ...
  sentenceDetails = sentenceDetails.map((details) => {
    let tokens = tokenizer.tokenize(details.sentence);
    ...
    return {
      ...details,
      sentiment: sentiment,
      rank: details.rank * (1 + sentimentRankAdjustment),
    };
  });
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      for (let j = 0; j < sentences.length; j++) {
        if (i !== j && similarityMatrix[i][j] > 0) {
          sum += similarityMatrix[i][j] * sentenceScores[j];
        }
      }
